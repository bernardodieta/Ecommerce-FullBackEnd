...
const saveAddressController = async (req, res) => {
  ...
  const dtoAddress = new AddressCreateDto(
    userId,
    country,
    state,
    city,
...
const getAddressById = async (req, res) => {
  ...
  if (!result) {
    req.logger.warning(
      `${req.method} en ${
        req.url
      } - Error: 'No se encontro una direccion para ese usuario' at ${new Date().toLocaleDateString()} - ${new Date().toLocaleTimeString()}`
    );
...
const getAddressByAddressId = async (req, res) => {
  ...
  if (!result) {
    req.logger.warning(
      `${req.method} en ${
        req.url
      } - Error: 'No se encontro una direccion con ese AddressId' at ${new Date().toLocaleDateString()} - ${new Date().toLocaleTimeString()}`
    );
...
const updateAddressController = async (req, res) => {
  ...
  if (!result) {
    req.logger.warning(
      `${req.method} en ${
        req.url
      } - Error: 'No se encontro una direccion para actualizar' at ${new Date().toLocaleDateString()} - ${new Date().toLocaleTimeString()}`
    );
    throw new NotFoundError("No se encontro una direccion para ese usuario");
  }
  response(res, 200, result);
...
const deleteAddress = async (req, res) => {
  const { id } = req.params;
  const result = await addressService.delAddresById(id);
  if (!result) {
    req.logger.warning(
      `${req.method} en ${
        req.url
      } - Error: 'No se encontro una direccion para eliminar' at ${new Date().toLocaleDateString()} - ${new Date().toLocaleTimeString()}`
    );
    throw new NotFoundError("No se encontro una direccion para ese usuario");
  }
  response(res, 200, result);
...
const TuningdeleteAddress = catchedAsync(deleteAddress);
...
export {
  TuningdeleteAddress as deleteAddress,
  TuningSaveAddressController as saveAddressController,
  TuningGetAddressById as getAddressById,
  TuningupdateAddressController as updateAddressController,
  TuninggetAddressByAddressId as getAddressByAddressId,
};
