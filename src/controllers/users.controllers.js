...
const registerUserController = async (req, res, next) => {
  try {
    const { first_name, last_name, date, tel, email, password, role } =
      req.body;
...
const reqPasswordReset = async (req, res, next) => {
  try {
    ...
    const token = jwt.sign({ email }, "resetpassword123456", {
      expiresIn: "1h",
    });
  ...
  try {
    ...
    if (bcrypt.compareSync(newPassword, user.password)) {
      throw new ConflictError(
        "La nueva password no puede ser igual a la anterior."
      );
    }
    ...
    if (error.name === "TokenExpiredError") {
      throw new NotFoundError(
        "El link ya expiro, vuelva a solicitar otro link."
      );
    }
    next(error);
  }
  ...
  try {
    ...
    if (user.documents && user.documents.length >= 4) {
      ...
      return res.status(400).json({
        error: true,
        message:
          "El usuario debe tener al menos 4 documentos cargados para ser promovido a PREMIUM.",
      });
    }
    const userUpda = {
      role: "PREMIUM",
