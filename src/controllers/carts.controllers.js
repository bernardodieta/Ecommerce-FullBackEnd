  ...
  try {
    ...
    if (result.matchedCount === 0) {
      req.logger.warning(
        `${req.method} en ${
          req.url
        } - Error: "No se encontró el producto o el usuario no tiene ese producto en su carrito" at ${new Date().toLocaleDateString()} - ${new Date().toLocaleTimeString()}`
      );
      throw new NotFoundError(
        "No se encontró el producto o el usuario no tiene ese producto en su carrito"
      );
    }
  ...
  try {
    ...
    if (!cart) {
      req.logger.warning(
        `${req.method} en ${
          req.url
        } - Error: "No se encontro un carrito para ese usuario" at ${new Date().toLocaleDateString()} - ${new Date().toLocaleTimeString()}`
      );
...
const prePurchase = async (req, res, next) => {
  try {
    ...
    const cart = await cartService.getCartByUserId(userId, req.logger);
    if (!cart.items.length) {
      req.logger.warning(
        `${req.method} en ${
          req.url
        } - Error: 'No hay productos para comprar' at ${new Date().toLocaleDateString()} - ${new Date().toLocaleTimeString()}`
      );
      throw new NotFoundError("No hay productos para comprar");
    }

    const { productosConStock, productosSinStock, totalPrice } =
      await processCartItems(cart.items, req);
...
const purchase = async (orderId, req, res, next) => {
  const { _id } = req.user;
  console.log("Llego a purchase");

  const order = await ordersService.getOrderById(orderId);
  const { products, productssinStock } = order;
 ...
 // const sendEmail = sendEmailConfirm(order, res);
  ...
  if (productssinStock.length >= 1) {
    response(
      ...
      201,
      order,
      `Algunas articulos sin stock no se incluyeron en su orden de compra.${sinStock}`
    );
  } else {
...
const processCartItems = async (items, req) => {
  ...
  const validProductPromises = productPromises.filter(
...
const updateStockAndRemoveFromCart = async (
  products,
  userId,
  res,
  req,
  ...
  const updatePromises = products.map(async (item) => {
    const productToUpdate = await productService.getProductById(
      item.product,
      req.logger
    );
    ...
    const remProductBuy = await remProductbuy(
      userId,
      item.product,
      res,
      req,
...
const TuningupdateStockAndRemoveFromCart = catchedAsync(
  updateStockAndRemoveFromCart
);
