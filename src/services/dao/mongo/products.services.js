...
export default class ProductServicesDao {
  ...
  getAllProducts = async (filter, options, logger) => {
    ...
    if (!products) {
      logger.error(
        `Error al obtener todos los productos.' - at ${new Date().toLocaleDateString()} - ${new Date().toLocaleTimeString()}`
      );
  ...
  saveProduct = async (product, logger) => {
    ...
    if (!newProduct) {
      logger.error(
        `Error al guardar el producto.' - at ${new Date().toLocaleDateString()} - ${new Date().toLocaleTimeString()}`
      );
  ...
  getProductById = async (id, logger) => {
    ...
    if (!productFind) {
      logger.error(
        `Error al actualizar el stock del producto' - at ${new Date().toLocaleDateString()} - ${new Date().toLocaleTimeString()}`
      );
  ...
  delProduct = async (_id, logger) => {
    ...
    if (!deletedProduct) {
      logger.error(
        `Error al borrar el producto' - at ${new Date().toLocaleDateString()} - ${new Date().toLocaleTimeString()}`
      );
  ...
  updateProduct = async (productId, product, logger) => {
    try {
      console.log("Desde service", productId, product);
      ...
      if (!updatedProduct) {
        logger.error(
          `Error al actualizar producto - at ${new Date().toLocaleDateString()} - ${new Date().toLocaleTimeString()}`
        );
  ...
  valAdd = async (productId, _id, message, star, logger) => {
    ...
    let newStatus = true;
    const updatedStatus = await this.updateProductStatus(
      _id,
      productId,
      newStatus
    );
  ...
  puedoValorar = async (_id, productId) => {
    try {
      ...
      const user = await userModel.findOne({
        ...
        "purchasedProducts.product.productId": productObjectId,
      });

      if (!user) {
        return {
          canRate: false,
          message: "El usuario no tiene comprado este producto.",
        };
      }

      const purchasedProduct = user.purchasedProducts.find((pp) =>
        pp.product.productId.equals(productObjectId)
      );
      ...
      return {
        canRate: true,
        message: "El usuario puede valorar este producto.",
      };
    } catch (error) {
      console.error(
        "Error verificando si el usuario ya realizo la valoracion del producto:",
        error
      );
      throw error;
    }
